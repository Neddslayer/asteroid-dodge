
{
	...
	register GLuint r, g, b, a;
#endif
#ifdef INTERP_RGB
	register GLuint rinc, ginc, binc, ainc;
#endif
#ifdef INTERP_Z
...
#ifdef INTERP_RGB
#define RGB(x) x
#define RGBPIXEL ZB_set(pp, RGB_TO_PIXEL(r >> 8, g >> 8, b >> 8, a >> 8))
...
#else
#define RGBPIXEL ZB_set(pp, color)
...
#define DRAWLINE(dx, dy, inc_1, inc_2)                                                                                                                         \
	n = dx;                                                                                                                                                    \
	ZZ(zinc = (p2->z - p1->z) / n);                                                                                                                            \
	RGB(rinc = ((p2->r - p1->r) << 8) / n; ginc = ((p2->g - p1->g) << 8) / n; binc = ((p2->b - p1->b) << 8) / n; ainc = ((p2->a - p1->a) << 8) / n);           \
	A = 2 * dy - dx;                                                                                                                                           \
	dy = 2 * dy;                                                                                                                                               \
	dx = 2 * dx - dy;                                                                                                                                          \
	pp_inc_1 = (inc_1)*PSZB;                                                                                                                                   \
