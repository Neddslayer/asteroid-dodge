...
void ZB_plot(ZBuffer* zb, ZBufferPoint* p) {
	...
	if (zbps == 1) {
        ...
        if(zb->maxy < p->y) zb->maxy = p->y;
		if (ZCMP(zz, *pz)) {
#if TGL_FEATURE_BLEND == 1
			if (!zb->enable_blend)
				ZB_set(pp, RGB_TO_PIXEL(p->r, p->g, p->b, p->a));
			else
		...
		PIXEL col = RGB_TO_PIXEL(p->r, p->g, p->b, p->a);
...
void ZB_line_z(ZBuffer* zb, ZBufferPoint* p1, ZBufferPoint* p2) {
	...
	color1 = RGB_TO_PIXEL(p1->r, p1->g, p1->b, p1->a);
	color2 = RGB_TO_PIXEL(p2->r, p2->g, p2->b, p2->a);
